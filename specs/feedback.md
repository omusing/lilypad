...
|------------|-------------------|-----------------------------------------------------------------------------------------------------------------------------------|---------|
| 2026-04-24 | Timeline — Pain card layout | Unify pain card layout with medication card layout. Currently the pain card has badge top-left, time top-right, region label bottom-left — inconsistent with the med card. Target layout: pain badge left-anchored (same as now), region/quality description to the right of the badge, top-aligned (mirroring how med name sits next to the left border). Time moved to bottom-right of the pain card, so it visually meets the bottom-left time on the adjacent medication card across the vertical rule. This creates a unified time axis at the bottom of each row. See `specs/references/text_to_small (1).png`. | Pending |
